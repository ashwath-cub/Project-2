...
int new_pll_freq(int crystal_val, signed char prdiv_val, signed char vdiv_val)
{
  ...
  // First move to PBE mode so that the PLL frequency can be safely changed.
  ...
  // Now in PBE mode
  // System is clocked off the external clock.
  // Now update the PLL settings and wait for the PLL to re-lock
  // Configure MCG_C5
  temp_reg = MCG_C5;
  temp_reg &= ~MCG_C5_PRDIV0_MASK;
  temp_reg |= MCG_C5_PRDIV0(prdiv_val - 1);    //set PLL ref divider
  ...
  // Change CLKS mux to now select the PLL output
  MCG_C1 &= ~MCG_C1_CLKS_MASK; // clear CLKS to switch CLKS mux to select PLL as MCG_OUT
...
{
  unsigned char extal_high = 0;
  unsigned char xtal_high = 0;
  short i;

  // Configure EXTAL (PTA18) and XTAL PTA(19) as GPIO driving Low, no pull enabled
  FGPIOA_PCOR = 0xC0000; // clear PTA18 and 19 output data
  FGPIOA_PDDR |= 0xC0000; // set PTA18 and 19 as outputs
  PORTA_GPCHR = (0xC0000 | PORT_PCR_MUX(1)); // configure the pin muxes for GPIO
  // Drive EXTAL high
  FGPIOA_PSOR = 0x40000; // set PTA18 data out high
  // Configure XTAL as an Input, no pull up
  FGPIOA_PDDR &= ~(0x80000); // clear PTA19 data direction to make it an input
  // Wait for ~2 time constants
  for (i = 0 ; i < 300 ; i++) {}
  // Check if XTAL was pulled high
  if (FGPIOA_PDIR & 0x80000)
  {
    xtal_high = 1;
  }
  // Configure EXTAL and XTAL as GPIO driving low
  FGPIOA_PCOR = 0xC0000; // clear PTA18 and 19 data output
  FGPIOA_PDDR |= 0xC0000; // set PTA18 and 19 as outputs
  // Drive XTAL high
  FGPIOA_PSOR = 0x80000; // set PTA19 data out high
  // Configure EXTAL as Input, no pull up
  FGPIOA_PDDR &= ~(0x40000); // clear PTA18 data direction to make it an input
  // Wait for ~2 time constants
  for (i = 0 ; i < 300 ; i++) {}
  // Check if EXTAL was pulled high
  if (FGPIOA_PDIR & 0x40000)
  {
    extal_high = 1;
  }

  // Now configure both pins back to their default state
  PORTA_GPCHR = 0x000C0000; // clear pta18 and 19 mux values to default
  FGPIOA_PDDR &= ~(0xC0000);    // set PTA18 and 19 as inputs
  FGPIOA_PCOR = 0xC0000;        // clear PTA18 and 19 output data

  // If both rising and falling are true return 1, else return 0
  if (extal_high && xtal_high)
  {
    return 1;
  }
  else
